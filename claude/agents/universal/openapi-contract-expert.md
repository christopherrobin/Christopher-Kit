...
- Identify the generation direction: hand-authored spec, TSOA-generated, Fastify + Zod type provider, or framework-extracted.
- `grep -rn "jsonSchemaTransform\|ZodTypeProvider\|@fastify/swagger" --include="*.ts" src/` - Fastify/Zod spec generation
...
- Backend controller or route implementation -> backend-expert or fastify-expert
...
- **Spec is auto-generated and read-only**: Focus fixes on the source (TSOA decorators, Fastify Zod schemas, framework annotations) rather than editing the spec directly.
- **Spec generated by @fastify/swagger + Zod type provider**: The spec is derived from Zod schemas via `jsonSchemaTransform`. Fix schema issues in the Zod definitions, not the generated output. Delegate Fastify plugin/route concerns to fastify-expert.
