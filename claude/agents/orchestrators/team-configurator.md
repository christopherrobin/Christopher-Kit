...
5. **Write / update CLAUDE.md**
   ...
   - Inside the autogenerated section, add a `### Coding Philosophy` subsection with these principles, verbatim (skip any bullet that clearly cannot apply to the detected stack):
   ...
   - Do NOT generate "Sample Commands" or "Example Commands" sections with `@agent-name` syntax - mentions resolve only in interactive prompt input, so they are inert text inside a generated CLAUDE.md. Claude delegates via the Task tool automatically based on the assignment table.

6. **Report to user**
   - Show detected stack.
   - List the agents added or updated.
   - Note: Claude automatically delegates to specialists via the Task tool based on the assignment table. Do NOT output `@agent-name` syntax in CLAUDE.md content - it only resolves in interactive prompts.
