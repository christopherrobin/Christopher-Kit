...
> The agent status requires the [subagent tracking hook](../hooks/) installed at `~/.claude/hooks/track-subagents.sh` and wired to `SubagentStart`/`SubagentStop` in `settings.json`. Without it, this always shows `N/A`.
